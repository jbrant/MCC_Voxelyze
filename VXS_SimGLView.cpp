...
{	
	...
	// //TODO: build an openGL list 
	// int floorTilesPerVoxel = 2;

	// vfloat Size = pSim->LocalVXC.GetLatticeDim()/floorTilesPerVoxel;
	// vfloat sX = 1.5*Size;
	// vfloat sY = .866*Size;
	...
	// vfloat floorHeight = -0.00001;
	// vfloat floorSlope = pSim->pEnv->FloorSlope;
	// vfloat heightCoeff = 1;
	// vfloat latticeDim = pSim->pEnv->pObj->GetLatticeDim();

	// glNormal3d(0.0, 0.0, 1.0);
	// // int i= 0;
	// // int j = 0;
	// for (int i=-20*floorTilesPerVoxel; i <=30*floorTilesPerVoxel; i++){
	// 	for (int j=-40*floorTilesPerVoxel; j <=60*floorTilesPerVoxel; j++){
	...
	// 		//// nac: floor slope
	// 		// glBegin(GL_TRIANGLE_FAN);
	// 		// glVertex3d(i*sX, j*sY, i*sX*floorSlope);
	// 		// glVertex3d(i*sX+0.5*Size, j*sY, (i*sX+0.5*Size)*floorSlope);
	// 		// glVertex3d(i*sX+0.25*Size, j*sY+0.433*Size, (i*sX+0.25*Size)*floorSlope);
	// 		// glVertex3d(i*sX-0.25*Size, j*sY+0.433*Size, (i*sX-0.25*Size)*floorSlope);
	// 		// glVertex3d(i*sX-0.5*Size, j*sY, (i*sX-0.5*Size)*floorSlope);
	// 		// glVertex3d(i*sX-0.25*Size, j*sY-0.433*Size, (i*sX-0.25*Size)*floorSlope);
	// 		// glVertex3d(i*sX+0.25*Size, j*sY-0.433*Size, (i*sX+0.25*Size)*floorSlope);
	...
	// 		////nac: rugged floor:
	// 		glBegin(GL_TRIANGLE_FAN);
	// 		glVertex3d(i*sX, 			j*sY, 				(sin(latticeDim*i*sX)+			sin(latticeDim*j*sY))				*heightCoeff);
	// 		glVertex3d(i*sX+0.5*Size, 	j*sY, 				(sin(latticeDim*i*sX+0.5*Size)+ sin(latticeDim*j*sY))				*heightCoeff);
	// 		glVertex3d(i*sX+0.25*Size,	j*sY+0.433*Size, 	(sin(latticeDim*i*sX+0.25*Size)+sin(latticeDim*j*sY+0.433*Size))  *heightCoeff);
	// 		glVertex3d(i*sX-0.25*Size,	j*sY+0.433*Size, 	(sin(latticeDim*i*sX-0.25*Size)+sin(latticeDim*j*sY+0.433*Size))  *heightCoeff);
	// 		glVertex3d(i*sX-0.5*Size,	j*sY, 				(sin(latticeDim*i*sX-0.5*Size)+ sin(latticeDim*j*sY))				*heightCoeff);
	// 		glVertex3d(i*sX-0.25*Size,	j*sY-0.433*Size,	(sin(latticeDim*i*sX-0.25*Size)+sin(latticeDim*j*sY-0.433*Size))  *heightCoeff);
	// 		glVertex3d(i*sX+0.25*Size, 	j*sY-0.433*Size, 	(sin(latticeDim*i*sX+0.25*Size)+sin(latticeDim*j*sY-0.433*Size))  *heightCoeff);
	...
	// 		glBegin(GL_TRIANGLE_FAN);
	// 		glVertex3d(i*sX+.75*Size, 	j*sY+0.433*Size, (sin(latticeDim*i*sX+.75*Size)+ sin(latticeDim*j*sY+0.433*Size)) *heightCoeff);
	// 		glVertex3d(i*sX+1.25*Size, 	j*sY+0.433*Size, (sin(latticeDim*i*sX+1.25*Size)+sin(latticeDim*j*sY+0.433*Size)) *heightCoeff);
	// 		glVertex3d(i*sX+Size, 		j*sY+0.866*Size, (sin(latticeDim*i*sX+Size)+     sin(latticeDim*j*sY+0.866*Size)) *heightCoeff);
	// 		glVertex3d(i*sX+0.5*Size, 	j*sY+0.866*Size, (sin(latticeDim*i*sX+0.5*Size)+ sin(latticeDim*j*sY+0.866*Size)) *heightCoeff);
	// 		glVertex3d(i*sX+0.25*Size, 	j*sY+0.433*Size, (sin(latticeDim*i*sX+0.25*Size)+sin(latticeDim*j*sY+0.433*Size)) *heightCoeff);
	// 		glVertex3d(i*sX+0.5*Size, 	j*sY, 			 (sin(latticeDim*i*sX+0.5*Size)+ sin(latticeDim*j*sY))		   	*heightCoeff);
	// 		glVertex3d(i*sX+Size, 		j*sY, 			 (sin(latticeDim*i*sX+Size)+     sin(latticeDim*j*sY))			*heightCoeff);
	// 		glVertex3d(i*sX+1.25*Size, 	j*sY+0.433*Size, (sin(latticeDim*i*sX+1.25*Size)+sin(latticeDim*j*sY+0.433*Size)) *heightCoeff);
	// 		glEnd();
	...
	// // // nac: draw goal
	// // Size = pSim->LocalVXC.GetLatticeDim();
	// // sX = 1.5*Size;
	// // sY = .866*Size;
	// // Size *= 10;	
	...
	// // floorHeight = 0.0001;
	// // glNormal3d(0.0, 0.0, 1.0);
	// // i = pSim->currentGoalLocationX;
	// // j = pSim->currentGoalLocationY;
		
	// // // glColor4d(0.6, 0.7+0.2*((int)(1000*sin((float)(i+110)*(j+106)*(j+302)))%10)/10.0, 0.6, 1.0);
	// // glColor4d(1.0, 0.784, 0, 1.0);
	...
	// // // draw turned off goal
	// // j = pSim->otherGoalLocationY;
...
//			LocalVXC.GetLeafMat(VoxArray[SIndex].GetVxcIndex())->GetColorf(&R, &G, &B, &A);
		...
		case RVC_STATE:
			...
			{
				double phaseTime = sin(2*3.1415926f * (pSim->CurTime/pSim->VoxArray[SIndex].TempPeriod + pSim->VoxArray[SIndex].phaseOffset));
				double thisAmp = (pSim->VoxArray[SIndex].ampBaseline + pSim->VoxArray[SIndex].ampResponse * pSim->VoxArray[SIndex].stimuli);
				// double thisAmp = 1+pSim->VoxArray[SIndex].stimuli;
				// thisAmp = 1;
				// double thisTemp = pSim->VoxArray[SIndex].TempFact/pSim->VoxArray[SIndex].GetCTE() - 1;
				...
				// return CColor(0.5-0.5*pSim->VoxArray[SIndex].GetCurScale(),0.5+0.5*pSim->VoxArray[SIndex].GetCurScale(),0.0, 1.0);//
				if (pSim->VoxArray[SIndex].GetCurScale() > 0.01)  {  return CColor(0.0,1.0,0.0, 1.0); }
				if (pSim->VoxArray[SIndex].GetCurScale() < 0.01)  {  return CColor(1.0,0.0,0.0, 1.0); }
				if (pSim->VoxArray[SIndex].GetCurScale() == 0.01) {  return CColor(0.0,0.0,1.0, 1.0); }
