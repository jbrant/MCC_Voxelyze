...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	if (!pXML->FindLoadElement("numSynapses", &numSynapses)) numSynapses = 0;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				...
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					...
					{	
						...
						SetSynapseWeight(voxCounter,s,atof(dataArray[numSynapses*k+s].c_str()));
	...
	// nac: load amp baseline
	if (pXML->FindElement("AmplitudeBaseline")){ 
		int voxCounter = 0;
		InitAmpBaselineArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetAmpBaseline(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	// nac: load amp response
	if (pXML->FindElement("AmplitudeResponse")){ 
		int voxCounter = 0;
		InitAmpResponseArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetAmpResponse(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
