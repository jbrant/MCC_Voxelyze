...
//		if ((AngMomZPos && AngMom.z < 0) || (AngMomZNeg && AngMom.z > 0)) AngMom.z = 0;
	...
	if (pSim->updateNeuralNetNow)
	// if (true)
	{
		// pSim->pEnv->pObj->GetSynapseWeights()

		// //


		// for (int s=0; s < pSim->pEnv->pObj->GetNumSynapses(); s++)
		// {
		// 	std::cout << pSim->pEnv->pObj->GetSynapseWeight(VoxNum,s) << ", ";
		// }
		// std::cout << std::endl;
		...
		// neuron 1: CPG node
		neuronValues[1] = sin(2*3.1415926f * (pSim->CurTime/TempPeriod));

		// neuron 2: touch sensor
		if (GetCurGroundPenetration() > 0.0) { neuronValues[2] = 1.0; } 
										else { neuronValues[2] = -1.0; }

		// neuron 3: avearage local output sensor
		...
		for (int i=0; i<6; i++)
		{
			...
			// if (IAmInternalVox2(i)) std::cout << VoxNum << "=?" << InternalBondPointers[i]->GetpV1()->neuronValues[6] << std::endl;
			// else std::cout << VoxNum << "=?" << InternalBondPointers[i]->GetpV2()->neuronValues[6] << std::endl;
		...
		neuronValues[3] = totalLocalOutput/numBondsToAvg;

		// neuron 4 and 5: hidden nodes
		double oldHidden1 = neuronValues[4];
		double oldHidden2 = neuronValues[5];
		...
		for (int i=0; i<4; i++)
		{
			inputToHidden1 += neuronValues[i]*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,i);
			inputToHidden2 += neuronValues[i]*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,i+4);
		}
		// recurrent connections
		inputToHidden1 += oldHidden2*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,8);
		inputToHidden2 += oldHidden1*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,9);
		inputToHidden1 += oldHidden1*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,10);
		inputToHidden2 += oldHidden2*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,11);
		// upstream inputs from motors last time step
		inputToHidden1 += oldMotorOutput*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,12);
		inputToHidden2 += oldMotorOutput*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,13);

		neuronValues[4] = tanh(inputToHidden1);
		neuronValues[5] = tanh(inputToHidden2);

		// neuron 6: 
		neuronValues[6] = tanh(	neuronValues[4]*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,14)
							  + neuronValues[5]*pSim->pEnv->pObj->GetSynapseWeight(VoxNum,15) );

		// std::cout << neuronValues[0] << ", "  << neuronValues[1] << ", " << neuronValues[2] << ", " << neuronValues[3] << ", " << neuronValues[4] << ", " << neuronValues[5] << ", " << neuronValues[6] << std::endl;
	}
...
//		vfloat ThisTemp = pSim->pEnv->pObj->GetBaseMat(GetMaterialIndex())->GetCurMatTemp();
		...
		// nac: multiply by voxel dim!!!!! TODO!!!!
		// float stimuliThisVoxel = (pow(pow(pSim->currentGoalLocationX - Pos.x/pSim->pEnv->pObj->GetLatticeDim(),2)+pow(pSim->currentGoalLocationY - Pos.y/pSim->pEnv->pObj->GetLatticeDim(),2),0.5));
		// float stimuliCenter = (pow(pow(pSim->currentGoalLocationX - pSim->SS.CurCM.x/pSim->pEnv->pObj->GetLatticeDim(),2)+pow(pSim->currentGoalLocationY - pSim->SS.CurCM.y/pSim->pEnv->pObj->GetLatticeDim(),2),0.5));
		// stimuli = tanh(stimuliThisVoxel - stimuliCenter);
		// std::cout << stimuliThisVoxel - stimuliCenter << " -- "<< stimuli << std::endl;

		// TempFact = (1+(ThisTemp - TempBase)*ThisCTE);	//To allow selective temperature actuation for each different material
		// TempFact = (1+((ampBaseline+ampResponse*stimuli)*TempAmplitude*sin(2*3.1415926f * (pSim->CurTime/TempPeriod + phaseOffset)))*ThisCTE); 
		// TempFact = (1+(TempAmplitude*sin(2*3.1415926f * (pSim->CurTime/TempPeriod)))*ThisCTE); 
		TempFact = (1+(TempAmplitude*neuronValues[6])*ThisCTE); 
		// std::cout << "old: " << sin(2*3.1415926f * (pSim->CurTime/TempPeriod)) << std::endl;
		// std::cout << "new: " << neuronValues[6] << std::endl;
		// std::cout << std::endl;


		// std::cout << "TempFact:" << TempFact << std::endl;
		// std::cout << "TempAmplitude:" << TempAmplitude << std::endl;
		// std::cout << "TempPeriod:" << TempPeriod << std::endl;
		// std::cout << "CurTime:" << pSim->CurTime << std::endl;
	...
	if ( (oldScale > 0) == (Scale > 0) ) { pSim->outputSwitchCounter++; }
...
{
	vfloat Penetration = 0.5*Scale - Pos.z + Pos.x*pSim->pEnv->FloorSlope;
