...
void CVX_Sim::ClearAll(void) //Reset all initialized variables
{
	...
	CurTime = (vfloat)0.0;
	CmInitialized = false;
	motionZeroedTime = 999999;
	CurStepCount = 0;
	DtFrozen = false;

	fitness1 = -999;
	outputSwitchCounter = 0;
...
//#endif
	...
	// {
	// 	for (int s=0; s < pEnv->pObj->GetNumSynapses(); s++)
	// 	{
	// 		std::cout << pEnv->pObj->GetSynapseWeight(i,s) << ", ";
	// 	}
	// 	std::cout << std::endl;
	// }
...
//	for (int k=0; k<iT; k++) BondArrayCollision[k].ResetBond();
	...
	for (int i=0; i<NumVox(); i++) 
	{
		// VoxArray[i].ampBaseline = pEnv->pObj->GetAmpBaseline(i);
		// VoxArray[i].ampResponse = pEnv->pObj->GetAmpResponse(i);
		...
		VoxArray[i].neuronValues.clear();
		for (int s=0; s<7; s++) // nac num neurons hard coded!!!
		{
			// std::cout << "s: " << s << std::endl;
			VoxArray[i].neuronValues.push_back(0.0);
		}
	}
...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
	if ((not CmInitialized) and CurTime > InitCmTime )
	{
		...
		for (int i=0; i<NumVox(); i++) 
		{
			...
			// numSamples++;pEnv->goalLocationY
		}
		// std::cout << "height after initialization: " << COMZ << std::endl;
		// pEnv->goalLocationY = pEnv->goalLocationY + IniCM.y/LocalVXC.GetLatticeDim();
		// pEnv->goalLocationX = pEnv->goalLocationY + IniCM.x/LocalVXC.GetLatticeDim();
		// pEnv->goalLocationX = 100;
		// std::cout << "IniCM.y: " << IniCM.y/LocalVXC.GetLatticeDim() << std::endl;
		// std::cout << "IniCM.x: " << IniCM.y/LocalVXC.GetLatticeDim() << std::endl;
		// std::cout << "IniCM.y/LocalVXC.GetLatticeDim(): " << IniCM.y/LocalVXC.GetLatticeDim() << std::endl;
		// std::cout << "goalLocation:" << pEnv->goalLocationX << ", " << pEnv->goalLocationY << std::endl;
		CmInitialized = true;

		if (fitness1 < 0)
		{
			currentGoalLocationY = pEnv->goalLocationY + IniCM.y/LocalVXC.GetLatticeDim();
			otherGoalLocationY = -pEnv->goalLocationY + IniCM.y/LocalVXC.GetLatticeDim();
			currentGoalLocationX = pEnv->goalLocationX + IniCM.x/LocalVXC.GetLatticeDim();

			// std::cout << "goalLocation:" << currentGoalLocationX << ", " << currentGoalLocationY << ", o: " << otherGoalLocationY << std::endl;
		}
	}

	if (fitness1 < 0 and CurTime > StopConditionValue)
	{
		fitness1 = pow(pow(currentGoalLocationX-SS.CurCM.x/pEnv->pObj->GetLatticeDim(), 2)+pow(currentGoalLocationY-SS.CurCM.y/pEnv->pObj->GetLatticeDim(), 2),0.5);
		currentGoalLocationY = -pEnv->goalLocationY + IniCM.y/LocalVXC.GetLatticeDim();
		otherGoalLocationY = pEnv->goalLocationY + IniCM.y/LocalVXC.GetLatticeDim();
		currentGoalLocationX = pEnv->goalLocationX + IniCM.x/LocalVXC.GetLatticeDim();
		// std::cout << "goalLocation:" << currentGoalLocationX << ", " << currentGoalLocationY << ", o: " << otherGoalLocationY << std::endl;
		ResetSimulation();
		// StopConditionValue = StopConditionValue/2.0;
	}
	...
	// if (MotionZeroed and CurTime < motionZeroedTime)
	// {
	// 	motionZeroedTime = CurTime;
	// 	std::cout << "motion zeroed at: " << CurTime << std::endl;
	// }

	// if (KinEHistory[0]+KinEHistory[1]+KinEHistory[2] < 0.01)
	// {
	// 	std::cout << "keneticEnergy: " << "stopped" << std::endl;
	// }
	// else
	// {
	// 	std::cout << "keneticEnergy: " << KinEHistory[0]+KinEHistory[1]+KinEHistory[2] << std::endl;
	// }
...
//#pragma omp parallel for
	...
	// nac: neural net update
	updateNeuralNetNow = false;
	if (CurTime - timeOfLastNeuralUpdate >= pEnv->GetTempPeriod()/5) //last term is neural updates per actuation period (make into parameter)
	{
		updateNeuralNetNow = true;
		timeOfLastNeuralUpdate = CurTime;
	}
...
//	Vec3D<> Ab = VoxArray[3].GetCurAngle();
	...
	// std::cout << "numSwitches: " << outputSwitchCounter << std::endl;
